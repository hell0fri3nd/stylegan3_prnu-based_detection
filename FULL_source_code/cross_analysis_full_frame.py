...
if __name__ == "__main__":
    ...
    if ans == "s":
        print('Loading NEW StyleGAN images')
        ...
        print('Loading StyleGAN OLD images')
        ffo_dirlist = np.array(sorted(glob(r'/content/drive/MyDrive/WIP/images/stylegan3_generated/*.png')))[:num_of_imgs]
        #ff_device = np.array([os.path.split(i)[1].rsplit('0', 1)[0] for i in ff_dirlist])[:num_of_imgs]
        print('Done!')
        #print(ff_device)
        #print(ff_dirlist)
        ...
        print('Computing fingerprints for StyleGAN OLD')
        #fingerprint_stylegan_psi1 = sorted(np.unique(ff_device))
        #print(fingerprint_stylegan_psi1)
        ffo = []
        imgs = []
        for img_path in ffo_dirlist:
            im = Image.open(img_path)
            # im = im.resize((2000, 3008), Image.ANTIALIAS)
            im_arr = np.asarray(im)
            if im_arr.dtype != np.uint8:
            ...
            im_cut2 = prnu.cut_ctr(im_arr, (512, 512, 3))
            imgs += [im_cut2]
        ffo += [prnu.extract_multiple_aligned(imgs, processes=1)]
        ...
        print('Computing residuals ffo_dirlist')
        ...
        pool = Pool(cpu_count())
        ffo_w = pool.map(prnu.extract_single, imgs)
        pool.close()
    
        ffo_w = np.stack(ffo_w, 0)
        print(ffo_w.shape)
        ...
        ffo_ff_corr = pd.DataFrame()
        #nat_ffo_corr = pd.DataFrame() # l'ho tolta tanto non ha senso farla
        ...
        print("Correlation mean of Fake - Fake Pic", ff_ff_corr.Corr_Value.mean())
         ...
         # Correlation fake - fakeold
        print('\nComputing correlation natural - fake')
        ffo_ff_corr = pd.DataFrame(prnu.aligned_cc(ff, ffo_w[[0]])['ncc'])
        ffo_ff_corr.columns = ["Corr Value"]
        for i in range(num_of_imgs):
            b = pd.DataFrame(prnu.aligned_cc(ff, ffo_w[[i]])['ncc'])
            ffo_ff_corr = ffo_ff_corr.append(b)
    
        ffo_ff_corr = ffo_ff_corr.iloc[1:]
        ffo_ff_corr.columns = ["Corr_Value", "Sıl"]
        ffo_ff_corr.drop("Sıl", inplace=True, axis=1)
        ffo_ff_corr.Corr_Value.hist()
        print("Correlation mean of fake - Fake old Pic", ffo_ff_corr.Corr_Value.mean())
        ...
        np.savetxt('NEW_ffo_ff_output_NEW_DATASET.txt',ffo_ff_corr)
        np.savetxt('NEW_nat_nat_output_NEW_DATASET.txt',nat_nat_corr)

    else:
        ff_ff_data = '/content/drive/MyDrive/WIP/stylegan3_prnu-based_detection/NEW_ff_ff_output_NEW_DATASET.txt'
        ffo_ff_data = '/content/drive/MyDrive/WIP/stylegan3_prnu-based_detection/NEW_ffo_ff_output_NEW_DATASET.txt'
        nat_ff_data = '/content/drive/MyDrive/WIP/stylegan3_prnu-based_detection/NEW_nat_ff_output_NEW_DATASET.txt'
        nat_nat_data = '/content/drive/MyDrive/WIP/stylegan3_prnu-based_detection/NEW_nat_nat_output_NEW_DATASET.txt'
        
        ff_ff_corr = pd.DataFrame(data = np.loadtxt(ff_ff_data))
        ffo_ff_corr = pd.DataFrame(data = np.loadtxt(ffo_ff_data))
    ...
    sns.distplot(ffo_ff_corr, label="StyleGAN3 OLD vs NEW", color="purple",kde=False, hist_kws=dict(alpha=0.5), norm_hist=True, bins=100)
    ...
    sns.distplot(ffo_ff_corr, label="StyleGAN3 OLD NEW", color="purple", hist_kws=dict(alpha=0.5), norm_hist=True, bins=100)
    plt.legend()
    plt.savefig('/content/drive/MyDrive/WIP/NEW_DATASET_ffo_ff_corr_graph'+str(os.getpid())+'.png', dpi=1200)
    plt.show()
